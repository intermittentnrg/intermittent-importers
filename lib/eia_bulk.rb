...
module EiaBulk
  class Base
    ...
    def self.copy
      ...
      ranges = ActiveRecord::Base.connection.select_rows("SELECT range_start,range_end,chunk_name FROM timescaledb_information.chunks WHERE hypertable_schema='intermittency' AND hypertable_name='#{self::TARGET_MODEL.table_name}' ORDER BY range_start")
  ...
  class Demand < Base
    ...
    def self.copy_sql(range_start, range_end)
      <<~SQL
        ...
        SELECT a.id,time-INTERVAL '1 hour',value FROM #{BULK_TABLE} ebl
  ...
  class Interchange < Base
    ...
    def self.copy_sql(range_start, range_end)
      <<~SQL
        ...
        SELECT from_area.id,to_area.id,time-INTERVAL '1 hour',value FROM #{BULK_TABLE} ebi
  ...
  class Generation < Base
    ...
    def self.copy_sql(range_start, range_end)
      <<~SQL
        ...
        SELECT apt.id,time-INTERVAL '1 hour',value FROM #{BULK_TABLE} ebg
