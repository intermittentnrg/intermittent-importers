...
<pre><A HREF="/public/public-data/datafiles/">[To Parent Directory]</A><br><br> Sunday, August 21, 2022  1:02 AM      1684350 <A HREF=\"/#{datafile_name}\"></A>
  ...
  describe :parse_time do
    ...
    it do
      expect(Out::Price).to receive(:run).with(array_including(hash_including(time: Time.new(2023, 8, 30, 8))),
    ...
    context 'NSW1' do
      it 'has expected price' do
        expect(Out::Price).to receive(:run).with(array_including(hash_including(value: 29_999)), anything, anything,
                                                 anything)
        subject
      end
    end
