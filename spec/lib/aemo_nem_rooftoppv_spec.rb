...
<pre><A HREF="/public/public-data/datafiles/">[To Parent Directory]</A><br><br> Sunday, August 21, 2022  1:02 AM      1684350 <A HREF=\"/#{datafile_name}\"></A>
      ...
      let(:zip_body) do
        create_zip_file(body, 'PUBLIC_ROOFTOP_PV_ACTUAL_MEASUREMENT_20230101000000_0000000396168830.csv', mtime)
      end
      ...
      it do
        expect(Out::Generation).to receive(:run).with(anything, Time.new(2022, 12, 31, 14, 0), Time.new(2022, 12, 31, 14, 5),
                                                      'aemo')
        subject.cli(args)
      end
    end
