...
#country = Area.find_by!(code: ARGV.shift, source: EntsoeApi::Unit.source_id)

ARGV.each do |area_code|
  area = Area.find_by!(code: area_code, source: EntsoeApi::Unit.source_id)
  (from..to).each do |date|
    SemanticLogger.tagged(country: area.code) do
      e = EntsoeApi::Unit.new(area, from: date, to: date+1.day)
      e.process
    rescue
